...
while read -r tag jid; do
  ...
  case "$tag" in
    ...
    g7-chrome-*) m="shards-done=$(ls -1 $OUT/slurm-evalbshard-${jid}_*.out 2>/dev/null | xargs -r grep -l 'SHARD-COMPLETE' 2>/dev/null | wc -l) tb=$(cat $OUT/slurm-evalbshard-${jid}_*.out 2>/dev/null | grep -c Traceback)" ;;
  esac
  echo "$tag $jid [$q] $m"
